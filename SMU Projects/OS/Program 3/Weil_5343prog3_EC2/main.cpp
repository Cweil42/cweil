...
bool Request2(int tid, int numTools){
    pthread_mutex_lock(&toolMut);
    for(int i = 0; i < k; i++){
        if(seatedToolsNeededMatrix[tid][i] > totalTools[i]){
            for(int i = 0; i < numTools; i++)
...
void releaseATool(int tid){
    ...
    if(acquired.empty()) {
        pthread_mutex_unlock(&toolMut);
        pthread_mutex_lock(&printMut);
        cout << seatedNames[tid] + " was denied but had nothing to release" << endl;
        pthread_mutex_unlock(&printMut);
        return; // no tools to give back
    }
    int randomNum = acquired[rand() % acquired.size()];
    seatedToolsAcquired[tid][randomNum]--;
    seatedToolsNeededMatrix[tid][randomNum]++;
    seatedToolsNeededList[tid].push_back(randomNum);
    totalTools[randomNum]++;
    pthread_mutex_unlock(&toolMut);

    pthread_mutex_lock(&printMut);
    cout << seatedNames[tid] + " was denied and released " << to_string(randomNum) << endl;
    pthread_mutex_unlock(&printMut);
}
...
void *runner(void *threadid) {
    ...
    struct timespec sleepTwoSec = {2, 0}; // Sleep for 2 seconds
    struct timespec sleepOneSec = {1, 0}; // Sleep for 2 seconds
    while(needMoreTables){ // While all playeres aren't finished
        while(!seatedToolsNeededList[tid].empty()) {
            ...
            if (t % 2 == 1 || seatedToolsNeededList[tid].size() == 1) {
                ...
                int q = seatedToolsNeededList[tid].size();
                numTools = 2 + (t/25) % (q - 1);
                for(int i = 0; i < numTools; i++)
                    listOfToolsLine += " " + to_string(seatedToolsNeededList[tid][i]);
            }
            pthread_mutex_lock(&printMut);
            cout << listOfToolsLine << endl;
            pthread_mutex_unlock(&printMut);

            bool granted = Request2(tid, numTools);
            ...
            if(!granted){ // If not granted
                nanosleep(&sleepOneSec, NULL); // sleep for 2 seconds
                releaseATool(tid); // Release a random tool
            }
        ...
        nanosleep(&sleepTwoSec, NULL); // sleep for 2 seconds
